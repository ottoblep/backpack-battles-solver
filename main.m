...
function value = objectiveFunction(blocklist)
    value = 0;
    adjacencyMatrix = checkBlockConnections(blocklist);
    for item = 1:length(blocklist)
        itemMultiplier = 1;
        % Check for modifiers from other items
        for i = 1:length(blocklist)
            if adjacencyMatrix(item, i) ~= 1; continue; end
            itemMultiplier = itemMultiplier * blocklist(i).connection_modifier;
        end
        value = value + (blocklist(item).value * itemMultiplier);
    end
end 

function adjacencyMatrix = checkBlockConnections(blocklist)
    adjacencyMatrix = zeros(length(blocklist));
    ...
    for j = 1:length(blocklist)
        for i = 1:length(blocklist)
            ...
            for connectionIdx = 1:length(blocklist(j).connection_blocks) 
                connectionBlockPosition = blocklist(j).position + rotateBlock(blocklist(j).connection_blocks(connectionIdx,:), blocklist(j).rotation);
                % Check for overlap with each block of item i
                for k = 1:size(blocklist(i).blocks,1)
                    if connectionBlockPosition == blocklist(i).position + rotateBlock(blocklist(i).blocks(k,:), blocklist(i).rotation);
